...
class JitterDatabase(context: Context)
    ...
    fun deleteAll() {
        val db = writableDatabase
        db.execSQL("DELETE FROM ${JitterDatabaseContract.JitterEntry.TABLE_NAME}")
    }

    fun deleteRecord(existingId: Int) : Int {
        // Define the WHERE clause
        val selection = "${BaseColumns._ID} = ?"
        // Specify the arguments in placeholder order
        val selectionArgs = arrayOf(existingId.toString())
        // Delete and return the number of rows deleted
        return writableDatabase.delete(JitterDatabaseContract.JitterEntry.TABLE_NAME, selection, selectionArgs)
    }

    fun findById(jitterId: Int) : Cursor {
        // Define the array of column names to return
        val projection = arrayOf(
            BaseColumns._ID,
            JitterDatabaseContract.JitterEntry.COLUMN_NAME_LOGINNAME,
        ...
        // Define the parameter values for the WHERE clause
        val selectionArgs: Array<String>? = arrayOf(jitterId.toString())
        // Define the GROUP BY clause
        val groupBy: String? = null
        // Define the HAVING clause
        val having: String? = null
